...
customized version of minitaur w/ ball on its back, based on Erwin Coumans's gist
...
ballStartPos = [0.1,0,0.35*2.0]
ballStartOrn = [0.5,0.5,0.5,0.5]

class MinitaurBallBulletEnv(gym.Env):
  ...
  def reset(self):
    if self._hard_reset:
      ...
      self._ballId = self._pybullet_client.loadURDF("%s/urdf/custom_sphere.urdf" % rel_path,ballStartPos,ballStartOrn)
      ...
      self._pybullet_client.resetBasePositionAndOrientation(self._ballId,ballStartPos,ballStartOrn)
  ...
  def lost_ball(self):
    points = self._pybullet_client.getContactPoints(self._ballId, self._groundId);
    return len(points)>0
  ...
  def _termination(self):
    ...
    return self.lost_ball() or self.is_fallen() or distance > self._distance_limit
